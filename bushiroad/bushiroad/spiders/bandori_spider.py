...
songs_dir = 'songs'
expected_band_names_of_category_page = ['Poppin\'Party', 'Afterglow', 'Pastel*Palettes',
                                        'Roselia', 'Hello, Happy World!', 'Morfonica', 'RAISE A SUILEN', 'MyGO!!!!!', 'Others']
for name in expected_band_names_of_category_page:
    for category in song_categories_url_encoded:
        os.makedirs(os.path.join(songs_dir, clean(
            name), category), exist_ok=True)
...
class BandoriSpider(scrapy.Spider):
    ...
    songs_found = 0
    start_urls = [scheme + subdo + sld + tld + subdi +
                  category for category in song_categories_url_encoded]

    def parse(self, response: Response):
        category = self.get_song_category_of_category_page(response=response)
        band_names = self.get_band_names_of_category_page(
            response=response, category=category)
        # for each band, yield a request to each song link
        for band_name in band_names:
            song_links = self.get_song_links_for_band_of_category_page(
                response=response, band_name=band_name, category=category)
    ...
    def parse_song_page(self, response: Response, category: str, band_name: str):
        dl_links = self.get_dl_links_of_song_page(
            response=response, category=category, band_name=band_name)
        file_names = self.get_file_names_of_dl_links(
            response=response, dl_links=dl_links)  # url-decoded
        file_names = [clean(re.match(r'(.+)(\.[^.]+)', file_name).group(1).rstrip('.') + re.match(
            r'(.+)(\.[^.]+)', file_name).group(2)) for file_name in file_names]  # trailing dots stripped then cleaned: ab\cdefg....ogg -> ab%5Ccdefg.ogg
        # folder_name is shortest file_name with extension removed
        folder_name = self.get_folder_name(
            response=response, file_names=file_names)
        ...
        for (dl_link, file_name) in zip(dl_links, file_names, strict=True):
            ...
            if not os.path.exists(file_path):
                cb_kwargs = {'category': category,
                             'band_name': band_name, 'file_path': file_path}
    ...
    def get_song_category_of_category_page(self, response: Response):
        # original_songs, etc
        category: str = response.url.replace(
            'https://bandori.fandom.com/wiki/Category:', '')
        if category not in song_categories_url_encoded:
            self.logger.error(
                f'Unexpected category {category} extracted from category page url at {response.url}')
            raise ValueError(
                f'Unexpected category {category} extracted from category page url at {response.url}')
        return category
    ...
    def get_band_names_of_category_page(self, response: Response, category: str):
        names = response.xpath(
            '//tbody/tr/td[@class="navbox-group"]/text()').getall()
        for name in names:
            if name not in expected_band_names_of_category_page:
                self.logger.error(
                    f'Unexpected band "{name}" found from {category} category page at {response.url}')
                raise ValueError(
                    f'Unexpected band "{name}" found from {category} category page at {response.url}')
        if len(names) > 0:
            return names
            ...
            self.logger.error(
                f'Could not find any bands from {category} category page at {response.url}')
            raise ValueError(
                f'Could not find any bands from {category} category page at {response.url}')
    ...
    def get_song_links_for_band_of_category_page(self, response: Response, band_name: str, category: str):
        ...
        links = response.xpath(
            f'//td[text()="{band_name}"]/following-sibling::td[position()=1]/div/a/@href').getall()
        if len(links) > 0:
            self.logger.info(
                f'Parsing {len(links)} {category} song links for {band_name}')
            self.songs_found += len(links)
            return links
        else:
            self.logger.error(
                f'Could not find any songs for {band_name} from {category} category page at {response.url}')
            raise ValueError(
                f'Could not find any songs for {band_name} from {category} category page at {response.url}')
    ...
    def get_dl_links_of_song_page(self, response: Response, category: str, band_name: str):
        ...
        for xpath in xpaths:
            ...
            if dl_links_length > 0:
                ...
                return dl_links
        self.logger.error(
            f'Unable to get download links from song page {response.url} of {band_name} {category}')
        return dl_links

    def get_file_names_of_dl_links(self, response: Response, dl_links: list[str]):
        file_names = [urllib.parse.unquote(re.match(
            r'^(https://static\.wikia\.nocookie\.net/bandori/images/[^/]+/[^/]+/)([^/]+)', dl_link).group(2)) for dl_link in dl_links]
        file_names_length = len(file_names)
        dl_links_length = len(dl_links)
        if file_names_length != dl_links_length:
            self.logger.error(
                f'Retrieved {file_names_length} file names although there are {dl_links_length} dl_links at {response.url}')
            raise ValueError(
                f'Retrieved {file_names_length} file names although there are {dl_links_length} dl_links at {response.url}')
        return file_names
    ...
    def closed(self, reason):
        ...
        self.logger.info(
            f'Links downloaded from and saved: {self.songs_downloaded_and_saved}')
